...
const createUser = asyncHandler(async (req, res) => {
  ...
  // });
    ...
    //     This link is valid till 10 minutes from now.<a href= 
    //     'http://localhost:8000/api/user/reset-password/${token}'> Click here</>`;
    //   const data ={
    //     to: email,
    //     text: "Hey friend",
...
// user cart
    const userCart = asyncHandler(async (req, res) => {
      ...
      try {
        let products = []
        const user = await User.findById(_id);
        // check if user already have pproducts in cart
        ...
        for (let i = 0; i < products.length; i++) {
          cartTotal = cartTotal+prodducts[i].price * products[i].count;
        }
...
module.exports = { createUser, loginUserCtrl, getAllUser, getAUser, deleteAUser,
   ...
   getUserCart, getWishlist, saveAddress,  updatedUser, resetPassword,
